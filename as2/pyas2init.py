...
def initialize():
    ...
    if not gsettings:
        ...
        gsettings['mdn_url'] = pyas2_settings.get('MDNURL', 'http://localhost:8080/as2/as2receive')
        gsettings['async_mdn_wait'] = pyas2_settings.get('ASYNCMDNWAIT', 30)
        gsettings['max_arch_days'] = pyas2_settings.get('MAXARCHDAYS', 30)
        gsettings['cert_dir'] = pyas2_settings.get('CERTDIR', os.path.join(gsettings['root_dir'], 'config/certificates'))
